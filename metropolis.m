...
%		everyone else.
curr_decrypted_txt = double2char(current_key(char2double(encrypted_txt)));
new_decrypted_txt = double2char(new_key(char2double(encrypted_txt)));

log_curr = logn_pr_txt(curr_decrypted_txt,pr_trans);
log_new = logn_pr_txt(new_decrypted_txt,pr_trans);

if(log_new >= log_curr)
    accept_new_key = 1; %definitely accept new key
    prob_accept = 1;
else
    prob_accept = exp(log_new - log_curr); %may accept new key
